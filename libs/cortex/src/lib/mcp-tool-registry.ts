...
/** Canonical names classified `write`; the annotation table must cover exactly these. */
export type CortexMcpWriteToolName = {
  [K in CortexMcpCanonicalToolName]: (typeof CORTEX_MCP_TOOL_KINDS)[K] extends 'write' ? K : never;
}[CortexMcpCanonicalToolName];

/**
 * MCP `ToolAnnotations` (spec 2025-03-26 onward; unchanged through 2026-07-28).
 *
 * Mirrors the spec interface field for field so a client typed against
 * `@modelcontextprotocol/sdk` reads it without translation. Every field is a *hint*:
 * the spec tells clients to treat them as untrusted, so they inform approval UX and
 * retry policy, never authorization — that stays with the token scope.
 *
 * The defaults matter: when a server sends no annotations a client assumes
 * `destructiveHint: true`, `idempotentHint: false`, `openWorldHint: true`. So an
 * unannotated read tool looks, to Claude Code or Cursor, exactly like a raw DELETE.
 */
export type CortexMcpToolAnnotations = {
  /** Reserved by the spec for a display title; ours goes on the Tool itself. */
  title?: string;
  /** True when the tool only reads. Derived from CORTEX_MCP_TOOL_KINDS, never declared twice. */
  readOnlyHint: boolean;
  /** True when the tool can delete rows or discard content wholesale (see the table). */
  destructiveHint: boolean;
  /** True when repeating the same call leaves the site in the same state. */
  idempotentHint: boolean;
  /** True when an argument may name an arbitrary external host (URL fetch, web search). */
  openWorldHint: boolean;
};

type CortexMcpToolBehaviour = {
  destructive: boolean;
  idempotent: boolean;
  openWorld: boolean;
};
...
/**
 * Behavioural hints for every WRITE tool, keyed by canonical name and typed against the
 * scope table so a tool added to CORTEX_MCP_TOOL_KINDS without a row here fails to
 * compile. Read tools are not listed: they are `destructive: false, idempotent: true`
 * by definition and only choose `openWorld` (see READ_TOOL_OPEN_WORLD).
 *
 * The rules, applied from the executor code rather than the tool description:
 *
 * - `destructive` is true when the tool can delete rows or discard existing content
 *   wholesale — a delete action or operation, a reset, or a "replace" mode that drops
 *   what is there — with no revision snapshot or Live Draft to restore from. Field-level
 *   overwrites of metadata and settings are not destructive; neither are page/post/
 *   product edits, which `recordRevision` snapshots, nor `rewrite_page_draft`, which only
 *   stages a draft. Mode-dependent tools take their worst mode, because the spec defines
 *   the hint as "MAY perform destructive updates".
 * - `idempotent` is true when a retry with identical arguments converges on the same
 *   state: creates are refused on a duplicate slug (`assertUniqueSlug`) or an existing
 *   translation before anything is written, drafts are delete-then-insert per parent,
 *   navigation "append" skips URLs already present, settings and languages are upserts,
 *   and deletes of something already gone are no-ops. It is false where a retry adds a
 *   second copy (`insert_content_block`, CSS "append", variant "append", raw inserts,
 *   URL imports that re-download into the media library) or where the output is
 *   generated by a model and so differs run to run (custom block definitions).
 * - `openWorld` is true only when an argument may name an arbitrary external host:
 *   fetching a URL, searching stock providers, importing an image by URL. Writes that
 *   merely register a stock-photo download as a side effect stay closed-world.
 */
export const CORTEX_MCP_WRITE_TOOL_BEHAVIOURS = {
  create_cms_page: { destructive: false, idempotent: true, openWorld: true },
  create_cms_post: { destructive: false, idempotent: true, openWorld: true },
  create_cms_product: { destructive: false, idempotent: true, openWorld: true },
  create_custom_block: { destructive: false, idempotent: false, openWorld: false },
  delete_cms_item: { destructive: true, idempotent: true, openWorld: false },
  delete_custom_block: { destructive: true, idempotent: true, openWorld: false },
  // A plan can carry delete_cms_item, insert_content_block and set_content_images, so it
  // inherits the worst of each.
  execute_cms_action_plan: { destructive: true, idempotent: false, openWorld: true },
  execute_database_action_plan: { destructive: true, idempotent: false, openWorld: false },
  execute_database_mutation: { destructive: true, idempotent: false, openWorld: false },
  finish_site_build: { destructive: false, idempotent: true, openWorld: false },
  insert_content_block: { destructive: false, idempotent: false, openWorld: false },
  manage_language: { destructive: false, idempotent: true, openWorld: false },
  manage_product_category: { destructive: true, idempotent: true, openWorld: false },
  manage_product_variants: { destructive: true, idempotent: false, openWorld: false },
  manage_site_script: { destructive: true, idempotent: true, openWorld: false },
  manage_site_theme: { destructive: false, idempotent: true, openWorld: false },
  // Overwrites the live blocks, but snapshots a revision first and consumes the draft,
  // so a second call finds nothing to publish.
  publish_content_draft: { destructive: false, idempotent: true, openWorld: false },
  reset_site_content: { destructive: true, idempotent: true, openWorld: false },
  // Logged as a new revision; the rollback can itself be rolled back.
  revert_site_script: { destructive: false, idempotent: true, openWorld: false },
  rewrite_page_draft: { destructive: false, idempotent: true, openWorld: false },
  save_site_brief: { destructive: true, idempotent: true, openWorld: false },
  set_content_images: { destructive: true, idempotent: false, openWorld: true },
  // Runs the reset, and each call (re)opens the time-boxed build session.
  start_site_build: { destructive: true, idempotent: false, openWorld: false },
  translate_content_bulk: { destructive: false, idempotent: true, openWorld: false },
  translate_page: { destructive: false, idempotent: true, openWorld: false },
  update_cms_item_field: { destructive: false, idempotent: true, openWorld: false },
  upload_media: { destructive: false, idempotent: false, openWorld: true },
  update_content_block: { destructive: false, idempotent: true, openWorld: false },
  update_current_cms_fields: { destructive: false, idempotent: true, openWorld: false },
  // Regenerates the whole definition from a description; nothing keeps the old layout.
  update_custom_block: { destructive: true, idempotent: false, openWorld: false },
  update_footer: { destructive: true, idempotent: true, openWorld: false },
  update_global_css: { destructive: true, idempotent: false, openWorld: false },
  update_navigation_bar: { destructive: true, idempotent: true, openWorld: false },
  update_section_column_block: { destructive: false, idempotent: true, openWorld: false },
  update_site_identity: { destructive: false, idempotent: true, openWorld: false },
} as const satisfies Record<CortexMcpWriteToolName, CortexMcpToolBehaviour>;

/** The two read tools whose arguments reach outside the site. */
const READ_TOOL_OPEN_WORLD: ReadonlySet<CortexMcpCanonicalToolName> = new Set([
  'fetch_url_content',
  'search_stock_photos',
]);
...
/**
 * Annotations for a tool or alias, or `null` for an unknown name. An alias carries the
 * annotations of the executor it forwards to — it cannot be safer than what it runs.
 */
export function getCortexMcpToolAnnotations(name: string): CortexMcpToolAnnotations | null {
  const canonical = resolveCortexMcpToolName(name);
  ...
  if (CORTEX_MCP_TOOL_KINDS[canonical] === 'read') {
    return {
      destructiveHint: false,
      idempotentHint: true,
      openWorldHint: READ_TOOL_OPEN_WORLD.has(canonical),
      readOnlyHint: true,
    };
  }

  const behaviour = CORTEX_MCP_WRITE_TOOL_BEHAVIOURS[canonical as CortexMcpWriteToolName];

  return {
    destructiveHint: behaviour.destructive,
    idempotentHint: behaviour.idempotent,
    openWorldHint: behaviour.openWorld,
    readOnlyHint: false,
  };
}
...
export type CortexMcpToolDefinition = {
  /** Always present: to a client, an absent annotation means "assume destructive". */
  annotations: CortexMcpToolAnnotations;
  description: string;
...
export function buildCortexMcpToolDefinitions(params: {
  ...
  for (const [name, entry] of Object.entries(registry)) {
    ...
    const annotations = getCortexMcpToolAnnotations(name);
  ...
  for (const [alias, config] of Object.entries(CORTEX_MCP_TOOL_ALIASES)) {
    ...
    const annotations = getCortexMcpToolAnnotations(alias);
